...
MedicareDataSource.prototype.FEATURES_ = new storeLocator.FeatureSet(
    new storeLocator.Feature('type-Fish', 'Fish'),
      new storeLocator.Feature('type-Thruster', 'Thruster')
);
...
MedicareDataSource.prototype.parse_ = function(csv) {
  ...
  for (var i = 1, row; row = rows[i]; i++) {
    ...
    features.add(this.FEATURES_.getById('type-' + row.type));
    ...
    var store = new storeLocator.Store(row.boardId, position, features, {
      title: this.join_([row.brand, row.feet, row.inches, row.type], ' '),
      address: row.city,
